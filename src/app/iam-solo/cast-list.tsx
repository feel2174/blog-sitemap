...
const castBySeason: Record<Season, CastGroup> = {
    30: publishedCast,
    31: publishedCast,
    32: {
        female: [
            { name: '영숙', bg: 'from-pink-100 to-rose-200', text: 'text-rose-600', hover: 'hover:border-rose-400 hover:shadow-rose-200' },
            { name: '정숙', bg: 'from-fuchsia-100 to-purple-200', text: 'text-purple-600', hover: 'hover:border-purple-400 hover:shadow-purple-200' },
            { name: '순자', bg: 'from-red-100 to-rose-100', text: 'text-red-500', hover: 'hover:border-red-300 hover:shadow-red-200' },
            { name: '영자', bg: 'from-pink-50 to-pink-200', text: 'text-pink-600', hover: 'hover:border-pink-300 hover:shadow-pink-200' },
            { name: '옥순', bg: 'from-rose-50 to-orange-100', text: 'text-orange-500', hover: 'hover:border-orange-300 hover:shadow-orange-200' },
            { name: '현숙', bg: 'from-orange-50 to-rose-100', text: 'text-orange-500', hover: 'hover:border-orange-300 hover:shadow-orange-200' },
            { name: '정희', bg: 'from-amber-50 to-pink-100', text: 'text-amber-600', hover: 'hover:border-amber-300 hover:shadow-amber-200' },
        ],
        male: [
            { name: '영수', bg: 'from-blue-100 to-cyan-200', text: 'text-blue-600', hover: 'hover:border-blue-400 hover:shadow-blue-200' },
            { name: '영호', bg: 'from-sky-100 to-blue-200', text: 'text-sky-600', hover: 'hover:border-sky-300 hover:shadow-sky-200' },
            { name: '영식', bg: 'from-indigo-100 to-purple-200', text: 'text-indigo-600', hover: 'hover:border-indigo-400 hover:shadow-indigo-200' },
            { name: '영철', bg: 'from-cyan-100 to-blue-100', text: 'text-cyan-600', hover: 'hover:border-cyan-300 hover:shadow-cyan-200' },
            { name: '광수', bg: 'from-slate-100 to-blue-200', text: 'text-slate-600', hover: 'hover:border-slate-400 hover:shadow-slate-200' },
            { name: '상철', bg: 'from-teal-100 to-sky-100', text: 'text-teal-600', hover: 'hover:border-teal-300 hover:shadow-teal-200' },
            { name: '경수', bg: 'from-violet-100 to-blue-100', text: 'text-violet-600', hover: 'hover:border-violet-300 hover:shadow-violet-200' },
        ],
    },
};

const seasons: Season[] = [30, 31, 32];

export default function CastList() {
    const [season, setSeason] = useState<Season>(30);
    const cast = castBySeason[season];
    const isPrivateSeason = season === 32;

    const handlePrivateLink = () => {
        alert('미공개');
    };

    const renderCastCard = (person: CastMember, tone: 'pink' | 'blue') => {
        const cardContent = (
            <>
                ...
                <div className={`flex items-center gap-1 text-sm font-medium ${tone === 'pink' ? 'text-pink-500' : 'text-blue-500'} opacity-80 group-hover:opacity-100 transition-opacity mt-2`}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                        <rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect>
                        <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path>
                        <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line>
                    </svg>
                    <span>{person.url ? '인스타그램 보기' : '인스타그램 공개 전'}</span>
                </div>
            </>
        );

        const className = `group bg-white rounded-3xl p-6 border-2 border-transparent shadow shadow-slate-100 ${person.hover} transition-all duration-300 flex flex-col items-center justify-center text-center focus:outline-none focus:ring-4 focus:ring-pink-100`;

        if (person.url) {
            return (
                <Link key={person.name} href={person.url} className={className}>
                    {cardContent}
                </Link>
            );
        }
        ...
        return (
            <button key={person.name} type="button" onClick={handlePrivateLink} className={className}>
                {cardContent}
            </button>
        );
    };
        ...
        <div className="w-full flex flex-col items-center">
            <div className="w-full max-w-5xl text-center mb-10 relative">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-slate-600 font-medium text-base sm:text-lg max-w-2xl mx-auto break-keep leading-relaxed mt-2">
                        ...
                        {isPrivateSeason ? '개인 인스타그램(Instagram) 프로필은 공개 확인 전까지 미공개로 안내됩니다.' : '개인 인스타그램(Instagram) 프로필 바로가기 주소 모음입니다.'}
                    </p>
                </div>
            </div>
            ...
            <div className="relative z-20 mb-16 p-2 bg-white/50 backdrop-blur-md rounded-3xl border border-pink-100 shadow-xl flex gap-2">
                {seasons.map((seasonNumber) => (
                    <button
                        ...
                        className={`px-8 py-3 rounded-2xl font-black text-lg transition-all duration-300 ${season === seasonNumber ? 'bg-gradient-to-r from-rose-500 to-pink-500 text-white shadow-lg shadow-pink-200' : 'text-slate-400 hover:text-rose-500 hover:bg-rose-50'}`}
                    >
                        {seasonNumber}기
                    </button>
                ))}
            ...
            <div className="w-full max-w-4xl relative z-10 transition-opacity duration-300">
                <section className="mb-16">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                        {cast.female.map((person) => renderCastCard(person, 'pink'))}
                    </div>
                </section>
                ...
                <section className="mb-12">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 justify-center">
                        {cast.male.map((person) => renderCastCard(person, 'blue'))}
                    </div>
                </section>
            </div>
            ...
            <div className="w-full max-w-3xl mt-10 p-6 bg-white/60 backdrop-blur-md rounded-3xl border border-slate-200/60 text-center relative z-10 shadow-sm">
                <p className="text-sm sm:text-base text-slate-500 leading-relaxed font-medium">
                    ...
                    {isPrivateSeason ? (
                        <>
                            공개되지 않은 SNS 주소는 실제 링크 대신 <strong className="text-rose-500">미공개 알림</strong>으로 표시됩니다.
                        </>
                    ) : (
                        <>
